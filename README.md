...
No server, no account, no install. The engine is a TypeScript library; a UI is not shipped yet.
...
**Status:** Stages 1–3 of the engine are in the library (wired core, access points as wired devices, multi-WAN). Stage 4 has the estimate/trace seam and optional `Radio.channel`; coverage is not modelled. Call `send` / `runFlow` from tests or your own code — there is no canvas yet. Who it is for is in [`docs/PRODUCT.md`](docs/PRODUCT.md); the spec is [`docs/SPEC.md`](docs/SPEC.md); decisions in [`docs/adr/`](docs/adr/); vocabulary in [`CONTEXT.md`](CONTEXT.md).
