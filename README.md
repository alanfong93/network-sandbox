...
**Status:** Stages 1–3 of the engine are in the library (wired core, access points as wired devices, multi-WAN). Stage 4 has the estimate/trace seam and optional `Radio.channel`; coverage is not modelled. A first browser UI (Stage 5, forms plus an SVG canvas) places presets, edits ports, sends a frame and shows the engine's hops, then steps a replay token along those hops. Drop, port-click cabling and drag live on the canvas under [ADR 0029](docs/adr/0029-canvas-is-a-ui-view-and-editor.md). Who it is for is in [`docs/PRODUCT.md`](docs/PRODUCT.md); the spec is [`docs/SPEC.md`](docs/SPEC.md); decisions in [`docs/adr/`](docs/adr/); vocabulary in [`CONTEXT.md`](CONTEXT.md).
...
5. **Browser UI** - started. Palette of presets, port inspector with separate PVID and untagged-VLAN controls, send a frame and read the hops, import/export sandbox JSON, SVG canvas view and builder (drop, port-click link, drag), hop replay (play/step, no verdict). Forms UI stays. The topology is the file; layout is an optional envelope sidecar, never Topology
