...
5. **Browser UI** - started. Palette of presets, port inspector with separate PVID and untagged-VLAN controls, send a frame and read the hops, import/export sandbox JSON, SVG canvas **view** of boxes and cables, hop replay (play/step, no verdict). Forms UI stays; canvas authoring (drop/drag) is still owed under [ADR 0029](docs/adr/0029-canvas-is-a-ui-view-and-editor.md). The topology is the file; layout is an optional envelope sidecar, never Topology
