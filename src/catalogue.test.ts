...
describe('catalogue', () => {
  it('carries all 26 rows with stable ids', () => {
  ...
  it('marks row 20 as stage 2 and rows 24-26 as stage 3, every other row as stage 1', () => {
    for (const row of CATALOGUE) {
      if (row.id === 20) expect(row.stage).toBe(2);
      else if (row.id === 24 || row.id === 25 || row.id === 26) expect(row.stage).toBe(3);
