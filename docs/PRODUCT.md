...
1. Build the topology they actually have — managed and unmanaged switches with their
   market port count, routers at any depth with their real LAN and WAN jack counts,
   hosts, DHCP both router-based and standalone, a resolver box whose records they edit,
   and the Internet box their names resolve to — with no install, no account, and no server.
