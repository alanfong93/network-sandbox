...
| `src/dhcp.ts` | DHCP server and relay as functions — siblings of routing when mounted on a router, or standalone on a host-like chassis (`standaloneDhcpDecision`, reached through the walk's fallback dispatch). DISCOVER/OFFER/REQUEST/ACK are frames. An OFFER is `poolStart` from the matching scope. No lease record. |
