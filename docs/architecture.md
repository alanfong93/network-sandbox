...
| `src/nat.ts` | NAT function on a routing function. Masquerade out any iface a default route names (ADR 0021): the selector-aware pick for the frame's VLAN, the plain default, or another WAN reached by a longer-prefix static or selector route - the set skips defaults whose via sits behind a down link, so masquerade follows failover (ADR 0020). Port-forwards match `proto` and `outsidePort` against either default's public IP; a DNAT that lands back in the ingress VLAN's own subnet is hairpin - source rewritten to the ingress iface IP and the return session recorded, so the reply re-enters the router (ADR 0021). Sessions live on the run context. |
